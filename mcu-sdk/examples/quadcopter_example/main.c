 ...
 *   $CH id=0,name=Throttle,type=u16,unit=us       ← 上电发一次
 *   $CH id=1,name=Accel_X,type=i16,unit=raw        ← 上电发一次
 *   ...（共 13+ 个通道）
 ...
 *   4. PC 端打开串口后自动显示设备名和 13 个通道，实时更新数值
 */
#include "mcu_debug.h"
...
static void Quadcopter_Register_Channels(void)
{
    ...
    Debug_Register_Channel(1, "Roll",     "u16", "us");
    Debug_Register_Channel(2, "Pitch",    "u16", "us");
    Debug_Register_Channel(3, "Yaw",      "u16", "us");

    /* 加速度计（原始值，LSB） */
    Debug_Register_Channel(4, "Accel_X", "i16", "raw");
    Debug_Register_Channel(5, "Accel_Y", "i16", "raw");
    Debug_Register_Channel(6, "Accel_Z", "i16", "raw");

    /* 陀螺仪（原始值，LSB） */
    Debug_Register_Channel(7, "Gyro_X", "i16", "raw");
    Debug_Register_Channel(8, "Gyro_Y", "i16", "raw");
    Debug_Register_Channel(9, "Gyro_Z", "i16", "raw");

    /* 磁力计（原始值） */
    Debug_Register_Channel(10, "Mag_X", "i16", "raw");
    Debug_Register_Channel(11, "Mag_Y", "i16", "raw");
    Debug_Register_Channel(12, "Mag_Z", "i16", "raw");

    /* 气压计 + 温度 */
    Debug_Register_Channel(13, "Pressure",    "u32", "raw");
    Debug_Register_Channel(14, "Temperature", "u32", "raw");
}
